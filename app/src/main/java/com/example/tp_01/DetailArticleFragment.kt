...
class DetailArticleFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (vm.isArticleInDb(args.article) == true)
        {
            Log.i(TAG, "onViewCreated: L'article ${args.article.id} est dans la BDD");
            view.findViewById<CheckBox>(R.id.chkBox_Favoris).isChecked = true;
        }
        else
        {
            Log.i(TAG, "onViewCreated: L'article ${args.article.id} n'est pas dans la BDD");
